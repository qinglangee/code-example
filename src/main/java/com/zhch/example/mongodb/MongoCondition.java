...
public class MongoCondition {
	...
	DBCollection col;
	int limit = 10; 
	MongoCondition() throws UnknownHostException {
	...
	public void and() {
		...
		condition.put("age", new BasicDBObject("$gt", 3));
		condition.put("weight", new BasicDBObject("$gt", 5));
		db.getCollection("zhch").find(condition);
		
		pringResult(condition);
	}
	public void or() {
		// weight < 4 or weight =5
		BasicDBObject condition = new BasicDBObject();
		BasicDBList values = new BasicDBList();
		values.add(new BasicDBObject("weight", new BasicDBObject("$lt", 4)));
		values.add(new BasicDBObject("weight", 5));
	...
	public void orDiffField() {
		// weight < 4 or weight =5
		BasicDBObject condition = new BasicDBObject();
		BasicDBList values = new BasicDBList();
		values.add(new BasicDBObject("weight", new BasicDBObject("$lt", 4)));
		values.add(new BasicDBObject("age", new BasicDBObject("$gt", 8)));
	...
	public void multiAndPlusOr() {
		// (age > 5 and height < 6 ) or weight < 5
		BasicDBObject and = new BasicDBObject();
		BasicDBObject condition = new BasicDBObject();
		and.put("age", new BasicDBObject("$gt", 5));
		and.put("height", new BasicDBObject("$lt", 6));
		BasicDBList values = new BasicDBList();
		values.add(and);
		values.add(new BasicDBObject("weight", new BasicDBObject("$lt", 5)));
	...
	public static void main(String[] args) throws UnknownHostException {
		MongoCondition t = new MongoCondition();
		t.multiAndPlusOr();
	}
}
...
class User {
	...
	public String toString() {
		return "id:" + id + " age:" + age + " weight:" + weight + " height:" + height;
	}
}
